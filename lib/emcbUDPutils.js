...
function getBroadcastAddress(ifaceName){
	return new Promise((resolve, reject) => {
		var onNetmaskKnown = function(netmask, ipAddress){
			try{
				resolve(ip.or(ip.not(netmask), ipAddress));	// Broadcast Address per https://en.wikipedia.org/wiki/Broadcast_address
			} catch(ex){
				reject(ex)
			}
		}
		...
		if(!ifaceName){
			return localIpV4Address().then(function(ipAddress){
				for (var iface in interfaces){
					...
					if(filtered.length){
						return onNetmaskKnown(filtered[0].netmask, filtered[0].address)
					}
				}
			});
		} else {
			...
			throw new Error("Unable to retrieve netmask for iface " + ifaceName)
		}
	})
}
