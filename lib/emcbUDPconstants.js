...
// Determine our network mask - I'm sure there is a a cleaner way of doing this...
for (iface in interfaces){
    var filtered = interfaces[iface].filter(details => {
        return details.address === ip.address()
    })
    if(filtered.length){
        netmask = filtered[0].netmask
        break;
    }
}
...
var constants = {
    ...
    EMCB_UDP_MESSAGE_CODE_GET_DEVICE_DEBUG_DATA                  : 0x00FE,
...
constants.EMCB_UDP_MESSAGE_CODES[constants.EMCB_UDP_MESSAGE_CODE_GET_DEVICE_DEBUG_DATA]               = "GET_DEVICE_DEBUG_DATA"
