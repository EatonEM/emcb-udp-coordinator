...
class EmcbUDPbroadcastMaster extends EventEmitter {
    ...
    constructor(args){
        ...
        this.ipAddress            = EMCB_UDP_BROADCAST_ADDRESS                     // This should be a broadcast or multicast address for the devices (right now, it should always be EMCB_UDP_BROADCAST_ADDRESS)
    ...
    syncDeviceSequenceNumbers(){
        ...
        this._sequenceNumber = emcbUDPutils.incrementSequenceNumber(this._sequenceNumber, 101)
    ...
    _send(activeMessage){
        ...
        logger.verbose(chalk[color](`>>>> SENDING ${EMCB_UDP_MESSAGE_CODES[activeMessage.tx.messageCode].padStart(30, ">")} (${activeMessage.tx.message.length.toString().padStart(3, " ")} bytes)  to  ${ activeMessage.tx.ipAddress }:${this.port}` + (activeMessage.tx.ipAddress === EMCB_UDP_BROADCAST_ADDRESS ? " (BROADCAST)" : "") + (idDevice ? ` [${idDevice}]` : "")))
    ...
    _handleTimeout(activeMessage, ipAddress){
        ...
        if(!(activeMessage.responses && ipAddress in activeMessage.responses) && !(activeMessage.errors && ipAddress in activeMessage.errors)){
            ...
            if(this.devices[ipAddress] && this.devices[ipAddress].consecutiveErrorsAndTimeouts === 100){
                ...
                this.emit(EMCB_UDP_EVENT_DEVICE_REMOVED, this.devices[ipAddress])
        ...
        } else if(Object.keys(this.devices).length === 0 && ipAddress === this.ipAddress && activeMessage.expectedNumResponses === Number.MAX_SAFE_INTEGER) {   // We are trying to discover devices and it isn't working out...
    ...
    _onPacket(msg, rinfo){
        ...
        if(activeMessage === undefined){
            ...
            logger.warn(chalk[color](`<<< NO ACTIVE MESSAGE for ${EMCB_UDP_MESSAGE_CODES[applicationLayerParsedMessage.messageCode].padStart(30)} response (likely due to timeout)` + (this.idDevice ? ` for [${this.idDevice}]` : "") + `.  Guessing this message took ${(new Date()).getTime() - this.lastMessageSendTime}ms to reach us`))
        ...
        logger.verbose(chalk[color](`<<< HANDLING ${EMCB_UDP_MESSAGE_CODES[applicationLayerParsedMessage.messageCode].padStart(30)} (${msg.length.toString().padStart(3, " ")} bytes)  in  ${(new Date()).getTime() - this.lastMessageSendTime}ms`))
        ...
        if(this.devices[rinfo.address] === undefined && parsedData.idDevice){
            ...
            //TODO: BUG: Need to test the case of an IP address changing...

            try{

                var existingDevice = this.getDevice(parsedData.idDevice)

                this.devices[rinfo.address] = new EmcbUDPdeviceMaster({
                    master          : this,
                    idDevice        : parsedData.idDevice,
                    ipAddress       : rinfo.address,
                    port            : this.port,
                    udpKey          : this.unicastUDPKeys[parsedData.idDevice],
                ...
                if(existingDevice){
                    ...
                    this.emit(EMCB_UDP_EVENT_DEVICE_IP_ADDRESS_CHANGED, {   //TODO: Will this ever get called?  Likely we hit 100 consecutive timeouts, remove the device, and then pick it back up during our next discoverDevices command...  For a device to change IP addresses, it likely has restarted and therefore its sequence number is out of sync...
                        ...
                        newIPaddress: rinfo.address,
                        device: this.devices[rinfo.address]
                    })
                }

                this.devices[rinfo.address].on(EMCB_UDP_EVENT_DEVICE_DISCOVERED, data => {
        ...
        if(!(parsedData instanceof Error)){
            ...
            activeMessage.responses[rinfo.address] = JSON.parse(JSON.stringify(parsedData))  // Deep copy so that mutations in Promises don't muck around with our event emitters parsedData
