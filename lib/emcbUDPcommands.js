...
// These are shared functions across our broadcast Coordinator and individual device nodes
...
// Our "this" will get set to the appropriate EmcbUDPbroadcastCoordinator or EmcbUDPdeviceCoordinator with a .bind for each scoped command.
...
var self = {
    ...
    patchEvseConfigSettingsAndMode: function(options) {
        ...
        if ("offlineMode" in options && options.offlineMode !== undefined){
            var offlineModes = {
                "no-restrictions" : 1,
                "no-change" : 2
            }
        ...
        if ("apiConfiguration" in options && options.apiConfiguration !== undefined){
            ...
            if ("restrictions" in config && config.restrictions !== undefined){
                var restrictions = config.restrictions;

                if ("maxCurrent" in restrictions && restrictions.maxCurrent !== undefined){
                ...
                if ("maxEnergy" in restrictions && restrictions.maxEnergy !== undefined){
    ...
    // This wrapper allows you to extend any method to support async retries.  Look at EMCBudpDeviceCoordinator.setBreakerState for an example of how to use
    ...
    rejectOnTimeoutOrNonACK:  function(data) {
        ...
        // if(this.coordinator === undefined && Object.keys(data.responses).length !== Object.keys(this.devices).length)
        if(data.timeouts)
