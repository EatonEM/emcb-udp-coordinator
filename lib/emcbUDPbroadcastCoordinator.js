...
const EmcbUDPdeviceCoordinator = require('./emcbUDPdeviceCoordinator.js');
...
class EmcbUDPbroadcastCoordinator extends EventEmitter {
    ...
    constructor(args){
        ...
        this.devices              = {}   //[ipAddress] = EmcbUDPdeviceCoordinator instance
		...
		this.getCoordinatorIpAddress				      = emcbUDPutils.getIpAddress;

		this._sendWithRetries                	  = emcbUDPcommands.sendWithRetries.bind(this)

		if(!this.ipAddress){
			logger.debug("No broadcast Address provided to EmcbUDPbroadcastCoordinator constructor - attempting to set for interface " + (args.ifaceName ? args.ifaceName : "default"))
			emcbUDPutils.getBroadcastAddress(args.ifaceName)
				.then(broadcastAddress => {
					logger.debug("Setting EmcbUDPbroadcastCoordinator broadcast Address to "+ broadcastAddress)
	...
	createDevice(idDevice, ipAddress, unicastGetNextSequenceNumber = true){
		try{
			...
			var newDevice = new EmcbUDPdeviceCoordinator({
				coordinator          : this,
				idDevice        : idDevice,
				ipAddress       : ipAddress,
				port            : this.port,
			...
			if(newDevice.udpKey) {	// If we don't have a udpKey for the device, we won't successfully create one and we don't need to store anything about it
                ...
                // We are now calling this inside of EmcbUDPdeviceCoordinator so that it can be called before setNextSequenceNumber
    ...
    discoverDevices(nonce, count){
        ...
        // When each device is discovered, we will create a device instance to grab its sequence number, device id, and get it synced up to our coordinator sequence number
    ...
    syncDeviceSequenceNumbers(){
        ...
        for(var ipAddress in this.devices){
            ...
            var useCoordinatorSequenceNumber = this.devices[ipAddress].useCoordinatorSequenceNumber
            d.useCoordinatorSequenceNumber = false;  //TODO: THIS IS A BUG - THIS SHOULDN"T HAPPEN UNTIL THE MESSAGE IS PULLED OFF OF THE QUEUE!
            promises.push(d.setNextSequenceNumber(this._sequenceNumber)
                .then(data => {
                    d.useCoordinatorSequenceNumber = true;
                    return data
                })
                .catch(err => {
                    d.useCoordinatorSequenceNumber = useCoordinatorSequenceNumber //Restore it to what it was
                    throw err
                })
            )
    ...
    //  - We can then do the final validation steps, parse the message, and fulfill our promise / emit any events that we need to (for both the broadcast and relevant unicast coordinators)!
    send(messageCode, messageData, ipAddress, udpKey, sequenceNumberOverride, expectedNumResponses, timeoutMs=EMCB_UDP_MESSAGE_THROTTLE_TIME_MS){
        return new Promise(function(fulfill, reject){
            this._messageQueue.push({
                "tx": {
                    ipAddress: ipAddress,
                    sequenceNumber: sequenceNumberOverride, // If this is undefined, it will be overriden to the coordinator's internally tracked current value of this._sequenceNumber in _send.  If it is a function, it will be called at the moment we need a fresh sequenceNumber (which is what we use before the EmcbUDPdeviceCoordinators and individual EMCBs are synced up to the coordinator)
    ...
    // send the next available message from our _messageQueue
    // if we find a message we can send then send it and return it
    // if there are no messages we can send then return null
    _sendNextPossibleMessageFromQueue(){
        logger.debug("searching for the next message we can send. _messageQueue.length: " + this._messageQueue.length);

        if (this._messageQueue.length === 0){
            logger.debug("_messageQueue is empty");
            return null;
        }

        if (this.ipAddress in this._sentQueue){
            logger.debug("can't send any messages until broadcast message completes");
            return null;
        }

        for (let i = 0; i < this._messageQueue.length; i++) {
            var nextMessage = this._messageQueue[i];
        ...
        logger.debug("hit the end of _messageQueue");
        return null;
    }
    ...
    _processQueues(){
        ...
        // send as many messages out as we can (returns null when it runs out of messages that can be sent)
        while (this._sendNextPossibleMessageFromQueue() !== null){ };
        ...
        this.udpSocket.send(activeMessage.tx.message, this.port, activeMessage.tx.ipAddress, function(error) {
            if(error){
                ...
                if(activeMessage){
                    ...
                    //TODO: Figure out how to clean up the activeMessage in broadcast/deviceCoordinator - so fare, we've never seen this error hit...
        ...
        if(!(activeMessage.responses && ipAddress in activeMessage.responses) && !(activeMessage.errors && ipAddress in activeMessage.errors)){
            ...
            if(this.devices[ipAddress]){    // In the unique case of a broadcast message without any devices yet discovered, we don't want this to barf.
                ...
                if(this.devices[ipAddress].consecutiveErrorsAndTimeouts%11 === 10){
                    ...
                    lostDevice.useCoordinatorSequenceNumber = false
                        ...
                        .then(data => {
                            ...
                            return lostDevice.setNextSequenceNumber(this._sequenceNumber)
                                            .then(data => {
                                                ...
                                                lostDevice.useCoordinatorSequenceNumber = true
                                            })
                                            .catch(err => {
    ...
    _onPacket(msg, rinfo){
        ...
        if(start != EMCB_UDP_HEADER_START_NODE){
            // Not the start of a packet per our API spec, we can ignore
            logger.warn("First 4 bytes of message do not equal " + EMCB_UDP_HEADER_START_NODE)
            return;
        }
        ...
        if(this.devices[rinfo.address] === undefined && parsedData.idDevice){
			...
			logger.verbose("Creating EmcbUDPdeviceCoordinator Instance for " + rinfo.address)
        ...
        if(!(parsedData instanceof Error)){
			...
			if(this.devices[rinfo.address]){
				...
				if(this.devices[rinfo.address][activeMessage.tx.messageCode] && typeof this.devices[rinfo.address][activeMessage.tx.messageCode] == "function"){
					...
					// If there is a parser, it is up to that parser to emit any successful events for the individual device and the coordinator!
...
module.exports = EmcbUDPbroadcastCoordinator
