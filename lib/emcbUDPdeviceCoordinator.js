...
class EmcbUDPdeviceCoordinator extends EventEmitter {    // Any of our message codes and any EMCB_UDP_EVENT_* constants can be .on'd.

    constructor(args){
		super();

        if(args.udpKey == undefined){
			logger.warn("No udpKey provided for discovered device " + args.idDevice + " at IP address " + args.ipAddress + " in EmcbUDPdeviceCoordinator constructor")
			return
        }
        ...
        this.chalkColor         = args.chalkColor || EMCB_UDP_DEVICE_COLORS[Object.keys(this.coordinator.devices).length % EMCB_UDP_DEVICE_COLORS.length];
        ...
        this.activeMessage   = undefined	// Hold an activeMessage for unicast messages as a flag for the broadcast coordinator to use
        ...
        this.useCoordinatorSequenceNumber = false

        // There is probably a better way to do this, but in the current design, when EmcbUDPbroadcastCoordinator.createDevice calls this, the below code will automatically call setNextSequenceNumber
        // EmcbUDPbroadcastCoordinator.createDevice will then call getNextSequenceNumber (if unicastGetNextSequenceNumber is true),
        ...
        // Set our Sequence Number to align to our coordinator
        if(args.sequenceNumber || args.autoGetAndSetSeqNum){
            this.setNextSequenceNumber(args.sequenceNumber, 3)
                .then(data => {
                    ...
                    this.useCoordinatorSequenceNumber = true;
                    this.emit(EMCB_UDP_EVENT_DEVICE_DISCOVERED, {device: this})
                })
                .catch(err => {
                    //TODO: In a lot of cases when we see this log, we do actually have a correct sequence number - the packet that held the response just was lost.  Really we should unicast read the Sequence number and see if it is sycnced with the coordinator before giving up...
    ...
    send(messageCode, messageData, ipAddress, udpKey, sequenceNumberOverride, expectedNumResponses=1, timeoutMs){
        return this.coordinator.send(
            messageCode,
            messageData,
            this.ipAddress, // We maintain function definition compatibility with EmcbUDPbroadcastCoordinator, but we don't want this send being used for any arbitrary IP address, so we force it to the right thing here
            this.udpKey,    // We maintain function definition compatibility with EmcbUDPbroadcastCoordinator, but we don't want this send being used for any arbitrary IP address, so we force it to the right thing here
    ...
    _getNextSequenceNumber(){
        if(this.useCoordinatorSequenceNumber === true) return undefined
    ...
    _emit(messageCode, parsedData){
        ...
        setImmediate(function(){
            this.coordinator.emit(messageCode, parsedData);
        }.bind(this))
    }
...
module.exports = EmcbUDPdeviceCoordinator
